<?php
  ...
  $query = "SELECT cedulaf,cedula,p_nombref,s_nombref,p_apellidof,s_apellidof,fec_nacf,edadf,generof,estudiante,funcionario,inf_medf,p_nombre,p_apellido
            FROM medicamentos 
            ORDER BY cedulaf";
  ...
  $pdf->SetX(8);
   ...
   // <!-- Final de titulos -->
  $pdf->Cell(53,7,utf8_decode('Cedula del Funcionario'),1,0,'C',1);
  $pdf->Cell(54,7,utf8_decode('Nombre del funcionario'),1,0,'C',1);
  $pdf->Cell(54,7,utf8_decode('Apellido del funcionario'),1,0,'C',1);
  $pdf->Cell(35,7,utf8_decode('Cedula del hijo'),1,0,'C',1);
  ...
  $pdf->Cell(60,7,utf8_decode('Información Médica'),1,1,'C',1);
  // <!-- Final de titulos -->
  
  while ( $row = $resultado->fetch_assoc() )
  {
   $pdf->SetX(8);
   $pdf->Cell(53,7,utf8_decode($row['cedula']),1,0,'C',0);
   $pdf->Cell(54,7,utf8_decode($row['p_nombre']),1,0,'C',0);
   $pdf->Cell(54,7,utf8_decode($row['p_apellido']),1,0,'C',0);
   $pdf->Cell(35,7,utf8_decode($row['cedulaf']),1,0,'C',0);
   ...
   $pdf->Cell(60,7,utf8_decode($row['inf_medf']),1,1,'C',0);  
  }
  $pdf->Output();
?>